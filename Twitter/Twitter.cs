...
namespace Twitter
{
    public class TwitterAPI
    {
        ...
        /// <param name="accessToken">Optional access token.</param>
        /// <returns>List of tweets.</returns>
        public List<Tweet> SearchTweets(string keywords, int maxTweets = 2000, string resultType = "mixed", int countPerPage = 100, string language = "en", GeoParams geoParams = null, bool includeEntities = true, Update updater = null, string accessToken = null)
        {
            List<Tweet> t = new List<Tweet>();
            ...
            try
            {
                ...
                if (geoParams != null)
                {
                    requestUri += "&geocode=" + geoParams.GetLocationForSearchAPI();    // Add the location to get the tweets
                }
                jsonStr = GetRequest(requestUri);
